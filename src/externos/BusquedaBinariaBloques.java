...
public class BusquedaBinariaBloques extends AlgorithmWindow {
    ...
    public BusquedaBinariaBloques(JFrame parent) {
        super(parent, "Búsqueda Binaria por Bloques");
        logToTerminal("Sistema de Búsqueda Binaria por Bloques iniciado", "info");
        logToTerminal("Algoritmo: Búsqueda binaria entre y dentro de bloques", "info");
        logToTerminal("Requisito: Estructura debe estar ordenada", "warning");
    ...
    private void searchKey(ActionEvent e) {
        try {
            ...
            logToTerminal(String.format("Iniciando búsqueda binaria por bloques para: '%s'", key), "info");
            logToTerminal("=== PROCESO DE BÚSQUEDA BINARIA ===", "info");

            BlockStructure.SearchResult result = blockStructure.binarySearch(key);
    ...
    private void logSearchSteps(java.util.List<BlockStructure.SearchStep> steps) {
        ...
        for (BlockStructure.SearchStep step : steps) {
            logToTerminal(String.format("Paso %d: %s", stepNumber++, step.description), "info");
        }
    }
    ...
    private void animateSearch(java.util.List<BlockStructure.SearchStep> steps) {
        ...
        Timer timer = new Timer(800, null); // Más lento para búsqueda binaria
    ...
    protected void exportToCSV() {
        ...
        if (fileChooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            try (PrintWriter writer = new PrintWriter(new FileWriter(fileChooser.getSelectedFile()))) {
                ...
                writer.println("Algoritmo,Busqueda Binaria Bloques");
                ...
                writer.println("Ordenado," + blockStructure.isSorted());
