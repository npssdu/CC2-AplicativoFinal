...
public class MultinivelPrimario extends AlgorithmWindow {
    private JTextField keyField, valueField, blockPointerField, maxEntriesField;
    ...
    public MultinivelPrimario(JFrame parent) {
        super(parent, "Multinivel Primario");
        logToTerminal("Sistema de Índice Multinivel Primario iniciado", "info");
        logToTerminal("Características: Múltiples niveles, clave única, estructura jerárquica", "info");
        logToTerminal("Optimizado para grandes volúmenes de datos", "info");
    ...
    protected JPanel createConfigurationPanel() {
        ...
        configPanel.setBorder(BorderFactory.createTitledBorder("Configuración del Índice Multinivel Primario"));
        ...
        maxEntriesField.setText("4"); // Valor por defecto
        ...
        characteristicsArea.setBackground(new Color(240, 255, 240));
        ...
        characteristicsArea.setText(
                "• Estructura jerárquica con múltiples niveles\n" +
                        "• Nivel hoja contiene todas las entradas ordenadas\n" +
                        "• Niveles superiores contienen índices de navegación\n" +
                        "• Búsqueda eficiente desde nivel superior hacia hoja"
        );
    ...
    private void initializeStructure(ActionEvent e) {
        try {
            ...
            indexStructure = new IndexStructure("MULTINIVEL_PRIMARIO", maxEntries);
            indexVisualization.setIndexStructure(indexStructure);

            enableControls(true);

            logToTerminal("Índice multinivel primario inicializado", "success");
            logToTerminal(String.format("Máximo de entradas por nivel: %d", maxEntries), "info");
            logToTerminal("Estructura jerárquica lista para inserción", "info");
    ...
    private void insertEntry(ActionEvent e) {
        try {
            ...
            logToTerminal(String.format("Estructura actual: %d niveles, %d entradas totales",
    ...
    private void searchEntry(ActionEvent e) {
        try {
            ...
            logToTerminal(String.format("Buscando clave primaria multinivel: '%s'", key), "info");
            logToTerminal("=== PROCESO DE BÚSQUEDA MULTINIVEL ===", "info");
            ...
            if (result.found) {
                logToTerminal(String.format("¡Clave encontrada en nivel %d! Valor: '%s', Bloque: %d",
                        result.level, result.entry.value, result.entry.blockPointer), "success");
                indexVisualization.highlightEntry(result.level, result.position);
            } else {
                logToTerminal("Clave no encontrada en el índice multinivel", "error");
            }
    ...
    private void deleteEntry(ActionEvent e) {
        try {
            ...
            logToTerminal(String.format("Eliminando clave primaria multinivel: '%s'", key), "info");
            ...
            if (removed) {
                ...
                logToTerminal(String.format("Estructura actual: %d niveles, %d entradas restantes",
                        indexStructure.getLevelCount(), indexStructure.getEntryCount()), "info");
                updateIndexVisualization();
                clearFields();
            } else {
    ...
    private void showLevelInfo(ActionEvent e) {
        ...
        logToTerminal("=== INFORMACIÓN DE NIVELES ===", "info");
        ...
        for (int level = indexStructure.getLevelCount() - 1; level >= 0; level--) {
            ...
            logToTerminal(String.format("Nivel %d (%s): %d entradas",
            ...
            if (!levelEntries.isEmpty()) {
                ...
                logToTerminal(String.format("  Rango: %s ... %s", firstKey, lastKey), "info");
            }
        }

        int maxEntries = indexStructure.getMaxEntriesPerLevel();
        logToTerminal(String.format("Máximo configurado por nivel: %d entradas", maxEntries), "info");
    }
    ...
    private void animateSearch(java.util.List<IndexStructure.SearchStep> steps) {
        ...
        Timer timer = new Timer(1000, null); // Más lento para multinivel
    ...
    protected void exportToCSV() {
        ...
        fileChooser.setDialogTitle("Guardar Índice Multinivel Primario como CSV");

        if (fileChooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            try (PrintWriter writer = new PrintWriter(new FileWriter(fileChooser.getSelectedFile()))) {
                writer.println("#CONFIGURACION");
                writer.println("Algoritmo,Multinivel Primario");
                ...
                for (int level = 0; level < indexStructure.getLevelCount(); level++) {
                    ...
                    for (IndexStructure.IndexEntry entry : levelEntries) {
                        writer.println(String.format("NIVEL_%d,%s,%s,%d",
                                level, entry.key, entry.value, entry.blockPointer));
                    }
                }
                ...
                logToTerminal("Índice multinivel primario exportado exitosamente", "success");
    ...
    protected void importFromCSV() {
        JFileChooser fileChooser = new JFileChooser();
        fileChooser.setDialogTitle("Importar Índice Multinivel Primario desde CSV");
    ...
    protected void resetAll() {
        int option = JOptionPane.showConfirmDialog(this,
                "¿Está seguro de que desea resetear el índice multinivel primario?",
                "Confirmar Reset",
        ...
        if (option == JOptionPane.YES_OPTION) {
            ...
            maxEntriesField.setText("4");
            clearFields();
            enableControls(false);
            terminalArea.setText("");

            updateIndexVisualization();
            logToTerminal("Índice multinivel primario reseteado", "warning");
        }
    }
}
